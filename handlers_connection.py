"""Connection lifecycle for Retool Connector."""
...
@chat.function(
    "connect_retool",
    "Connect Retool account via credentials.",
    action_type="write",
    chain_callable=True,
    event="retool-connector.connect_retool",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_retool(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Retool Connector."""
    client = RetoolClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Retool Account",
...
@chat.function(
    "list_connections",
    "List connected Retool accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_retool",
    "Disconnect Retool account.",
    action_type="write",
    chain_callable=True,
    event="retool-connector.disconnect_retool",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_retool(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
